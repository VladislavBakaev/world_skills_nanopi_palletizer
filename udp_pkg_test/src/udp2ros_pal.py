...
class Udp2Motrorcortex(object):
    ...
    def mainLoop(self):
        #thread.start_new_thread( self.printParams, ("Thread-1", 0.01, ))
        ...
        if not msg==None:
            ...
            if(self.buttonStatus == False):
                if msg[0]=='p':
                    ...
                    self.move_robot.moveToPoint([float(list_[1]), float(list_[2]), float(list_[3]), float(list_[4])],self.UDP_SERVER_ADDRESS, self.UDP_PORT_OUT,self.udp_socket)
                    #self.move_robot.gripper(list_[5][0])
                    self.move_robot.compress(list_[5][0],self.port)  
...
class MoveRobot(object):
    ...
    def moveToPoint(self, point,ip,port,sock):
        global status
